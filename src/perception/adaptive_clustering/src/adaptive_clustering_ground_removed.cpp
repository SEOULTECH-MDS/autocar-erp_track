...
public:
  ...
  : Node("adaptive_clustering_ground_removed", options)
  {
    ...
    this->declare_parameter<float>("z_axis_max", 1.0);
    ...
    point_cloud_sub_ = this->create_subscription<sensor_msgs::msg::PointCloud2>(
      "velodyne_points_ground_removed", 1, std::bind(&AdaptiveClusteringGroundRemoved::pointCloudCallback, this, std::placeholders::_1));
  }

private:
  void pointCloudCallback(const sensor_msgs::msg::PointCloud2::SharedPtr ros_pc2_in) {
    if(print_fps_)if(reset_){frames_=0;start_time_=clock();reset_=false;}//fps
    ...
    if(print_fps_)if(++frames_>10){RCLCPP_INFO(this->get_logger(), "[adaptive_clustering_ground_removed] fps = %f, timestamp = %ld", float(frames_)/(float(clock()-start_time_)/CLOCKS_PER_SEC), clock()/CLOCKS_PER_SEC);reset_=true;}//fps
  }
